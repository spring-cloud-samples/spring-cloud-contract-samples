package com.example.security.oauth2.provider.token;
...
/**
 * <p>Utility interface for converting a user authentication to and from a map.</p>
 */
public class UserAuthenticationConverter
    ...
    /**
     * <p>Extract information about the user to be used in an access token (i.e. for resource servers).</p>
     *
    ...
    /**
     * <p>Inverse of {@link #convertUserAuthentication(Authentication)}. Extracts an Authentication from a map.</p>
     *
    ...
    public Authentication extractAuthentication(final Map<String, ?> authenticationAsMap) {
        notNull(authenticationAsMap, "authenticationAsMap");

        String userName = (String) authenticationAsMap.get(USERNAME);
        Map<String, Object> userDetailsMap = (Map<String, Object>) authenticationAsMap.get(USERDETAILS);
    ...
    private static UserDetails extractUserDetails(Map<String, Object> userDetailsMap) {
        return UserDetails.builder()
                          .username(MapUtils.getString(userDetailsMap, PROP_USERNAME))
                          .age(MapUtils.getInteger(userDetailsMap, PROP_AGE))
                          .build();
    }
    ...
    private static List<GrantedAuthority> extractGrantedAuthorities(Collection<String> authorities) {
        return AuthorityUtils.commaSeparatedStringToAuthorityList(StringUtils.collectionToCommaDelimitedString(authorities));
    }
