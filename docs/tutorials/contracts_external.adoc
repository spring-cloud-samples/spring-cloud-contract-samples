...
Since we have all contracts for our system in one place we can easily
create the documentation of the whole API. You can create the following test
that will generate a `contracts.adoc` file under `target/generated-snippets/` with
description of contracts and with the contract bodies as such.
